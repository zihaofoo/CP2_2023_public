...
all_predictions = np.load('grids_filtered_advisor_next_8_below_0.85.npy')

temp = np.load('grids_filtered_advisor_top_8_below_0.85.npy')
print(temp.shape)
print(all_predictions.shape)
all_predictions = np.vstack((all_predictions,temp))
pdb.set_trace()

i=2


top_six = all_predictions[14:20,:,:].astype(int)
print(top_six.shape)
final_predictions = top_six[0:1]
for j in (1,2,3,4,5):
    for k in range(2 ** j):
...
for l in range(37):
    final_predictions = np.vstack((final_predictions,top_six[5:6]))
print(np.array(final_predictions).shape)
