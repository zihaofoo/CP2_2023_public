...
np.savetxt("mask0.csv", preds0, delimiter=',', fmt='%d')

np.savetxt("mask1.csv", preds1, delimiter=',', fmt='%d')
np.savetxt("mask2.csv", preds2, delimiter=',', fmt='%d')
np.savetxt("mask3.csv", preds3, delimiter=',', fmt='%d')
np.savetxt("mask_total.csv", mask_total, delimiter=',', fmt='%d')
grids = load_grids() # Helper function we have provided to load the grids from the dataset
# loaded_data = np.load('grids_advisor2_good.npz')
# Access the arrays from the loaded data
# grids = loaded_data['arr_0']  # 'arr_0' is the default key used by np.savez
...
np.save(f"185858585.npy", final_submission)
