import sys
sys.path.append('/home/turbo/nicksung/cp2/CP2_2023_public')
...
"Functions for obtaining features"
...
" Model architecture"
def create_combined_model(num_conv_layers=2, conv_layer_size=64, num_dense_layers=2, dense_layer_size=128, learning_rate=0.001):
...
"Define the target function for smac hyperparameter tuning"
def target_function(num_conv_layers, conv_layer_size, num_dense_layers, dense_layer_size, learning_rate, batch_size, epochs, seed=None):
    model = create_combined_model(num_conv_layers, conv_layer_size, num_dense_layers, dense_layer_size, learning_rate)
    # If you want to use the seed for reproducibility, you can set it here for random operations or initializations.
    history = model.fit([grids_train, features_train], ratings_train, epochs=epochs, batch_size=batch_size, validation_split=0.1, verbose=0)
    # Return the negative validation loss as we want to maximize it
    return -history.history['val_loss'][-1]


"Load and prep data"
grids = load_grids() #Helper function we have provided to load the grids from the dataset
ratings = np.load("/home/turbo/nicksung/cp2/CP2_2023_public/datasets/scores.npy") #Load advisor scores
...
# Step 3: Define the hyperparameter configuration space
cs = ConfigurationSpace()
num_conv_layers = UniformIntegerHyperparameter("num_conv_layers", 1, 4)  # 1 to 4 convolutional layers
conv_layer_size = UniformIntegerHyperparameter("conv_layer_size", 32, 256)  # size between 32 and 128
num_dense_layers = UniformIntegerHyperparameter("num_dense_layers", 1, 4)  # 1 to 4 dense layers
dense_layer_size = UniformIntegerHyperparameter("dense_layer_size", 64, 256)  # size between 64 and 256
learning_rate = UniformFloatHyperparameter("learning_rate", lower=1e-5, upper=1e-2, log=True)
cs.add_hyperparameters([num_conv_layers, conv_layer_size, num_dense_layers, dense_layer_size])
cs.add_hyperparameters([learning_rate])
batch_size = CategoricalHyperparameter("batch_size", [32, 64, 128, 256])
cs.add_hyperparameter(batch_size)

# Add epochs as a uniform integer hyperparameter
epochs = UniformIntegerHyperparameter("epochs", 10, 100)
...
#     "deterministic": "true"
# })
scenario = Scenario(cs, deterministic=True, n_trials=2000)
smac = HyperparameterOptimizationFacade(scenario, target_function)
incumbent = smac.optimize()
optimal_configuration = incumbent.get_dictionary()
print("Optimal hyperparameters:", optimal_configuration)
