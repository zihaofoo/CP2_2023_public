...
all_predictions = np.load('re-check.npy')
...
for i in range(4):
    ...
    # pdb.set_trace()
    if i==3:
        final_predictions = all_predictions[18 : 19 ,:,:]
        final_predictions = np.vstack((final_predictions,all_predictions[19 : 20 ,:,:]))
        final_predictions = np.vstack((final_predictions,all_predictions[19 : 20 ,:,:]))
        for n in range(97):
            final_predictions = np.vstack((final_predictions,all_predictions[20 : 21 ,:,:]))
        print(np.array(final_predictions).shape)
        final_submission = final_predictions.astype(int)
